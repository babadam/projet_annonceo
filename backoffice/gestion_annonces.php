...
$gestion_annonce = $resultat -> fetch(PDO::FETCH_ASSOC);
...
foreach($gestion_annonce as $valeur){ // parcourt tous les enregistrements

    $contenu .= '<tr>'; // ligne pour chaque enregistrement

        foreach ($gestion_annonce as $indice => $valeur2) { // parcourt toutes les infos de chaque enregistrement
            if($indice == 'photo'){
        ...
        $contenu .= '<td><a href="supprimer_produit.php?id='.$gestion_annonce['id_annonce'].'">Supprimer<img src="../img/delete.png"></a></td>';
    $contenu .= '</tr>';
}
